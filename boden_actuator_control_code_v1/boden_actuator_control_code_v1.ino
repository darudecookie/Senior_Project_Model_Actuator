...
const int StepperServo_runtime_interval_microsecs[stepper_servo_axes] = {1000, 1000}; // interval for joints to loop at. this is an array as opposed to one value because you might want joints that have to move quickly or make fine adjustments like a wrist mechanism to refresh faster, not sure if this is really necessary though
...
const long PID_params[stepper_servo_axes][3] = { // pid params
    {1, 0, 0},
    {1, 0, 0}};
const float angle_offset_sign[stepper_servo_axes][2] = {{0, 1}, {0, 1}}; // angle offset for encoder input, first value is added and the second is multiplied by the encoder input
...
private:
  ...
  void check_current_angle() // function reads encoder values and then converts them to angle sw params
  {
    ...
    float angle = analogRead(joint_num) * angle_offset_sign[joint_num][1] + angle_offset_sign[joint_num][0];
